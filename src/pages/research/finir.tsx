...
function Resource({ href, title, detail, icon }: { href: string; title: string; detail: string; icon: "github" | "pypi" | "huggingface" | "book" }) {
  ...
  return <a href={href} target="_blank" rel="noopener noreferrer" className="finir-resource flex items-center justify-between gap-5 border-b border-[#d9e0e1] px-3 py-5 sm:px-4">
    <span className="flex items-center gap-4">{logo ? <span className="flex h-8 w-8 shrink-0 items-center justify-center"><Image src={logo.src} alt={logo.alt} width={24} height={24} className="h-auto max-h-6 w-auto" /></span> : <span className="flex h-8 w-8 shrink-0 items-center justify-center"><BookOpen className="h-4 w-4 text-[#527487]" aria-hidden /></span>}<span><strong className="block text-sm font-semibold text-[#24343b]">{title}</strong><span className="mt-1 block text-xs text-[#718087]">{detail}</span></span></span><ArrowUpRight className="h-4 w-4 shrink-0 text-[#527487]" aria-hidden />
  </a>;
}
